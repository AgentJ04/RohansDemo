trigger ConUpdatedEmail on Contact (before update) {
    ...
    try{
        ...
        */
        ...
        if(adxsett != null && adxsett.TSGADX__EA_Alerts__c && cons.size()>0 && !me.Profile.Name.contains('System Administrator') && changedRecords > 0) Messaging.sendEmail(new Messaging.SingleEmailMessage[]{mail});
    
        for(Contact con : changedContacts)
        {
            con.EA_Field_Changed_At__c = Datetime.Now();
        }
